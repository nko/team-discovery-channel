...
function request(host, path, callback){
    ...
    req.addListener('response', function(res){
        ...
        res.addListener('end', function(){
            try {
                callback(res.body, res.statusCode, path);
            } catch (err) {
                callback(false, res.statusCode, path);
            }
        });
    });
    req.end();
                ...
                if ( content && status == 200 ) {
                    ...
                    function runScripts() {
                        try {
                            ...
                            if ( sandbox.errors ) {
                                sys.puts(JSON.stringify({errors: sandbox['errors']}));
                            }
                            
                        } catch (e) {
                            sys.puts(JSON.stringify({error: e}));
                        }
                    }
                                        
                    var scriptCallback = function(content, status, path) {
                        scriptLoadCount += 1;
                        ...
                        if (scriptLoadCount == scriptCount && frameLoadCount == frameCount) { 
                            for (var key in loadedScripts) {
                                var struct = loadedScripts[key];
                                if (!struct.path.match(/^.*\/qunit[-\w]*\..*$/)) {
                                    scriptContent += "try {" + struct.source + "} catch(e) { var errors = errors || []; errors.push(\"" + path + ": \" + e); }"
                                }
                            }
                    ...
                    document.jQuery('script').each( function() {
                        ...
                        // Keep track of script execution order.
                        var struct = {
                            path: path,
                            src: ""
                        };
                        loadedScripts.push( struct );
